...
class ProstateImages(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        #print(self.df[self.df["idx"] == pt_id].drop(["idx", self.target_var], axis=1))
        #print(self.df[self.df["idx"] == pt_id].drop(["idx", self.target_var], axis=1).values)
        ddf = self.df[self.df["idx"] == pt_id].drop(["idx", self.target_var], axis=1).astype(float).to_numpy()
        cli_data = torch.squeeze(torch.Tensor(ddf)) if self.cli_size > 0 else None
        if self.cli_only:
...
class ProstateData:
    ...
    def preprocess(self, train_df, test_df, train_ids, config):
        ...
        '''    if self.config["normalization"]:
                ...
                train_df[col] = train_df[col].astype(bool) ^ tr_min[col].astype(bool)

                #train_df[col].astype(bool) ^= tr_min[col].astype(bool)
                train_df[col] /= (tr_max[col] ^ tr_min[col] + eps * (tr_max[col] == tr_min[col]))

                test_df[col] = test_df[col].astype(bool) ^ tr_min[col].astype(bool)

                #test_df[col].astype(bool) ^= tr_min[col].astype(bool)
